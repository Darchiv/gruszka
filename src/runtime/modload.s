...
;*                                                                           */
;*                                                                           */
;*     2018      Daniel Sawka - added exported symbols handling              */
;*                                                                           */
;*                                                                           */
...
; Exported globals. Search for "start" symbol.
; Simple mock. Skips symbols number and expects first symbol to be "start".
...
@L1:
        ...
        jsr     ReadByte
        ply
        cmp     start_symbol_txt,y
        bne     Error
        iny
        cmp     #$0
        bne     @L1
        
        ; Segment type expected to be 2 (text).
        jsr     ReadByte
        cmp     #$2
        bne     Error

        ; Start entry point address.
        jsr     ReadByte
        ldy     #MOD_CTRL::ENTRY
        sta     (Ctrl),y

        jsr     ReadByte
        ldy     #MOD_CTRL::ENTRY+1
        sta     (Ctrl),y
        jmp     DoneAll
